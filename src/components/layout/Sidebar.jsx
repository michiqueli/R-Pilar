...
const Sidebar = ({ isCollapsed, onToggle }) => {
  ...
  const filteredSections = SIDEBAR_SECTIONS.map(section => ({
    ...
    items: section.items.filter(item => {
      // Si el item no requiere rol, se muestra. 
      // Si lo requiere, verificamos que el usuario sea ADMINISTRADOR.
  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 overflow-y-auto overflow-x-hidden py-4 px-3 space-y-6">
        {filteredSections.map((section, idx) => ( // <-- Usamos filteredSections
