...
const ClientSearchable = ({ value, onChange }) => {
  const { t } = useTheme();
  ...
  useEffect(() => {
    const fetchSelected = async () => {
      if (value && (!selectedClient || selectedClient.id !== value)) {
  ...
  useEffect(() => {
    const searchClients = async () => {
      // Search regardless of open state if we have a search term, 
      // but usually we want to trigger on open or type.
      ...
      >
        ...
        {selectedClient && !isOpen ? (
          ...
          <input
            ...
            placeholder={selectedClient ? selectedClient.name : t('projects.searchClient')}
            value={search}
          ...
          <div className="max-h-[240px] overflow-y-auto p-1.5 space-y-1">
            {loading ? (
              ...
              clients.map((client) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {client.email && <div className="text-xs text-slate-400 truncate">{client.email}</div>}
                  </div>
              ...
              <div className="p-4 text-center text-sm text-slate-500">
                {search ? t('messages.no_clients_found') : t('projects.searchClient')}
              </div>
            )}
          </div>
...
export default ClientSearchable;
