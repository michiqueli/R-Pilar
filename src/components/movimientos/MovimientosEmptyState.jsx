...
const MovimientosEmptyState = ({ onCreate }) => {
  const { t } = useTheme();
  ...
  return (
    ...
    >
      <div className="w-20 h-20 rounded-full bg-gray-50 dark:bg-gray-800 flex items-center justify-center mb-6 shadow-inner">
        <ArrowRightLeft className="w-10 h-10 text-gray-400 dark:text-gray-500" strokeWidth={1.5} />
      </div>

      <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-3">
        {t('movimientos.noMovements')}
      </h3>

      <p className="text-gray-500 dark:text-gray-400 max-w-sm mx-auto mb-8 text-sm leading-relaxed">
        {t('movimientos.createFirst')}
      </p>

      <Button
        onClick={() => onCreate('gasto')}
        variant="primary"
