...
const InvestorsEmptyState = ({ onCreate }) => {
  const { t } = useTheme();
  ...
  return (
    ...
    >
      <div className="w-20 h-20 rounded-full bg-gray-50 dark:bg-gray-800 flex items-center justify-center mb-6 shadow-inner">
        <Users className="w-10 h-10 text-gray-400 dark:text-gray-500" strokeWidth={1.5} />
      </div>
      ...
      <Button
        onClick={onCreate}
        variant="primary"
        className="rounded-full bg-[#3B82F6] hover:bg-blue-700 text-white shadow-md hover:shadow-lg transition-all duration-200 px-6 py-2.5 h-auto text-sm font-medium"
      >
...
export default InvestorsEmptyState;
